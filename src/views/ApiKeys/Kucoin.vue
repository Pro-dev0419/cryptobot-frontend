<template>
	<DefaultLayout>
		<h2 class="text-3xl mb-2">Kucoin</h2>
					...
					<form class="space-y-4 md:space-y-6" action="#" @submit="handleSubmit">
						...
						<div class="flex items-center">
							<label for="apipassword"
								class="w-28 mb-2 text-sm font-medium text-gray-900 text-white">Api Password
								<span class="text-red-500">*</span></label>
...
export default defineComponent({
	...
	setup() {
		...
		const apiPassword = ref(null);
        const authStore = useAuthStore();
		const handleSubmit = async (e: any) => {
			e.preventDefault();
			if(apiKey.value && apiSecret.value) {
				const Kraken = await fetchWrapper.post(`${baseUrl}/apikey/kucoin`, { user_id: authStore.user.user.id, key: apiKey.value, secret: apiSecret.value, password: apiPassword.value});
			}
