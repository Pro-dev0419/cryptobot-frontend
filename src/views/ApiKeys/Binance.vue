<template>
	<DefaultLayout>
		<h2 class="text-3xl mb-2">Binance</h2>
	...
	setup() {
		...
		const handleSubmit = async (e: any) => {
			...
			if(apiKey.value && apiSecret.value) {
				const Binance = await fetchWrapper.post(`${baseUrl}/apikey/binance`, { user_id: authStore.user.user.id, key: apiKey.value, secret: apiSecret.value});
			}
